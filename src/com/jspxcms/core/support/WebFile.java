...
public class WebFile {
	...
	public WebFile(File file, String root, String contextPath) {
		this.file = file;
		this.root = root;
	...
	public WebFile getParentFile() {
		return new WebFile(file.getParentFile(), root, contextPath);
	}
	...
	public List<WebFile> listFiles(FileFilter filter) {
		...
		for (int i = 0, len = files.length; i < len; i++) {
			wf = new WebFile(files[i], root, contextPath);
	...
	public static void sort(List<WebFile> list, String sort, String sortDir) {
		Comparator<WebFile> comp;
		if ("name".equals(sort)) {
			comp = "desc".equals(sortDir) ? NAME_DESC : NAME_ASC;
		} else if ("lastModified".equals(sort)) {
			comp = "desc".equals(sortDir) ? LASTMODIFIED_DESC
					: LASTMODIFIED_ASC;
		} else if ("type".equals(sort)) {
			comp = "desc".equals(sortDir) ? TYPE_DESC : TYPE_ASC;
		} else if ("length".equals(sort)) {
			comp = "desc".equals(sortDir) ? LENGTH_DESC : LENGTH_ASC;
		} else {
			comp = NAME_ASC;
		}
